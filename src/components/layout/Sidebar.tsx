          ...
          <div className="bg-white rounded-2xl shadow-xl mx-4 max-w-md w-full max-h-[80dvh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--card-border)' }}>
              <div>
                ...
                <p className="text-[11px] text-[var(--muted)]">v0.5.2 — April 2026</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 text-xs space-y-4">
              <ChangelogSection title="v0.5.2 — Clickable Stat Tile Filters" items={[
                'Stat tiles (Flagged, Accepted, Remaining) are now 3-way clickable filters: off → include → exclude',
                'Composable filters — combine multiple tile filters simultaneously',
                'Visual states: colored accent bar + checkmark (include), red strikethrough + X (exclude)',
                'Removed duplicate filter tab bar, kept Variance and Uncounted as toggle pills',
              ]} />
              ...
              <ChangelogSection title="v0.4.2 — Channel-Aware Round Replacement" items={[
                ...
                'Scan records queries now paginate past Supabase 1000-row limit',
              ]} />
              <ChangelogSection title="v0.4.1 — Recount Engine + Recon Polish" items={[
              ...
              <ChangelogSection title="v0.3.15 — Value Deviation + Dual Rings" items={[
                ...
                'Stock Valuation = sum of pastel_qty × unit_cost across all rows',
                'Removed "X of X parts counted" line — replaced with stock valuation R-value',
              ]} />
              <ChangelogSection title="v0.3.14 — Action Alignment + Live C1/C2 Deviation" items={[
                'Action icons (accept, flag, exclude) right-aligned in their column',
                'Deviation summary now updates in real-time when you toggle C1/C2 per row',
                'Matches per-row toggle state instead of just the initial variance-based default',
              ]} />
              <ChangelogSection title="v0.3.13 — Table Sizing + Meta Position" items={[
                '"Showing X of Y parts" moved above the table so it is visible while scrolling',
                'Reduced table max-height so the page itself no longer scrolls (internal scroll only)',
              ]} />
